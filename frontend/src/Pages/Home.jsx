...
function Home() {
 ...
 const [comment,setComment]=useState({});
   ...
   const getPosts=useCallback(async ()=>{
      ...
      const result=await fetch(`${url}/post/getallposts`,{
        method:"GET",
        headers:{
...
const setDateInString=(date)=>{
  ...
  return (
    <div className='relative  w-screen h-[90vh] flex flex-col overflow-auto scrollbar-hide'>
      <div><Story/></div>
        <div className='flex flex-col pt-1 items-center  w-full '>
  {post.length>0 &&  <div  className='w-full md:w-[60vw] lg:w-[80vw] xl:w-[80vw] 2xl:w-[60vw] flex flex-col  gap-5 items-center max-h-[80vh]   md:max-h-screen   overflow-auto scrollbar-hide'>
      {
        post.map((ele)=>(
 ...
 allPostInteraction[ele._id]?.type==="Likes" ?    ( allPostInteraction[ele._id]?.likedBy?.map((likes)=>(
              ...
              <div className='flex flex-col sm:flex-row sm:gap-2 sm:items-center sm:justify-start lg:flex-col lg:items-start lg:gap-0   2xl:flex-row 2xl:gap-2 justify-center 2xl:items-center mb-2'><NavLink to={`/${comment.userId.username}`}> <h1 className='text-[0.8rem] sm:text-lg font-serif text-blue-500'>{comment.userId.username}</h1> </NavLink> <h1 className='text-[0.5rem] opacity-95 sm:text-sm font-serif'>{setDateInString(comment.createdAt)}</h1></div>  
